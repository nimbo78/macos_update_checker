...
- 🐳 Docker: в volume `./data` (монтируется в `/app/data`)
- 🐍 Локально: в папке проекта `./data` или `./`

При обновлении эти файлы **не удаляются**.

**Что сохраняется в базе данных:**
- 📦 История всех найденных релизов macOS
- 📊 История проверок
- 📢 **Динамические цели уведомлений** (добавленные через `/addtarget`)

> 💡 Динамические таргеты, добавленные через бота, сохраняются в БД и не пропадут при обновлении!
...
- [ ] Таргеты сохранились: отправить `/targets` в Telegram
